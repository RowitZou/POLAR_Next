"""
MaterialRewardManager V3 — verl 0.7.0 外挂 reward manager（无需修改 verl 源码）

设计思路：
  - 继承官方 RateLimitedRewardManager，利用其三层并发控制
    (max_concurrent / max_rpm / max_tpm / timeout)
  - 复用 reward_material_v3.py 中的所有评分逻辑（LLM Judge + Anti-Hack Rubrics），不重复实现
  - 仅需向父类注入一个符合 verl 签名的单条同步评分函数

V3 变更（基于 V2）：
  - **零预测惩罚**：对于回归任务，如果 gold 值非零但模型预测了 0，
    直接给最低分 0.0。其余情况仍按 MAE 分数计算。
    该机制针对模型以预测 0 作为安全默认值来最大化期望 reward 的 hack 行为。
...
加载方式（shell script 中配置）：
    ...
    reward_model.reward_loop_module_path="../src/reward/mix_env/material_reward_manager_v3.py"
...
from reward_material_v3 import (  # noqa: E402
...
def _material_compute_score(
    ...
    2. LLM Classification/Regression Judge → llm_score
       - V3: 回归任务中，若 gold != 0 且 pred == 0，直接给最低分 0.0
...
class MaterialRewardManager(RateLimitedRewardManager):
    """材料科学任务奖励管理器 V3（verl 0.7.0，importlib 外挂，无需修改 verl 源码）。
    ...
    * 回归任务 → LLM 提取数值 → MAE → reward ∈ [0, 1]
      - V3: 若 gold != 0 且 pred == 0，直接给最低分 0.0
    * Anti-Hack Rubric → 独立评判伪造引用/证据等 → 全部通过才保留 llm_score

    V3 变更：
    * 零预测惩罚：回归任务中，gold 非零而模型预测为 0 → 直接最低分 0.0
    * 其余逻辑与 V2 完全一致
    """

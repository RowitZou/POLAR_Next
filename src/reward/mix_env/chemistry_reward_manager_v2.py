"""
ChemistryRewardManager V2 — verl 0.7.0 外挂 reward manager（无需修改 verl 源码）

设计思路：
  - 继承官方 RateLimitedRewardManager，利用其三层并发控制
    (max_concurrent / max_rpm / max_tpm / timeout)
  - 复用 reward_chemistry_v2.py 中的所有评分逻辑（Rule + LLM Judge），不重复实现
...
加载方式（shell script 中配置）：
    ...
    reward_model.reward_loop_module_path="../src/reward/mix_env/chemistry_reward_manager_v2.py"
...
# 从 reward_chemistry_v2.py 导入纯逻辑构件（不含任何配置常量）
from reward_chemistry_v2 import (  # noqa: E402
    judge_template,            # LLM Judge 提示词模板（含 CoT）
    judge_template_no_cot,     # LLM Judge 提示词模板（不含 CoT）
...
def _judge_process(query: str, thinking: str, prediction: str, task_type: str, client: OpenAI) -> dict:
    ...
    与 reward_chemistry_v2.judge_process_async 逻辑完全相同，
...
class ChemistryRewardManager(RateLimitedRewardManager):
    ...
    * ``timeout``         — 单条请求超时（默认 600s）
    ...
    该函数复用 ``reward_chemistry_v2.py`` 中的所有原子构件。

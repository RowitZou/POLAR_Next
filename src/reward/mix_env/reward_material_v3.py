"""
Reward function V3 for Material Science (MatBench) RL tasks.
...
V3 Changes (based on V2):
  - **Zero-prediction penalty**: For regression tasks, if the gold value is non-zero
    but the model predicts 0, the score is set to 0.0 (minimum) directly.
    This targets the observed "zero-bias hack" where the model defaults to 0.0
    as a safe answer to maximize expected reward.
    Only when gold != 0 AND pred == 0 is the penalty applied;
    otherwise normal MAE-based scoring is used.
...
# ===== V3: Zero-prediction penalty threshold =====
ZERO_PRED_EPSILON = 1e-6
...
async def judge_regression_async(
    ...
    V3: If gold value is non-zero but model predicts 0, returns minimum score (0.0) directly.
    """
    ...
    try:
        ...
        # V3: Zero-prediction penalty — if gold is non-zero but model predicts 0,
        # give minimum score directly to penalize the zero-bias hack.
        if abs(gold_val) > ZERO_PRED_EPSILON and abs(pred_val) < ZERO_PRED_EPSILON:
            return {"score": 0.0, "predicted_value": pred_val, "gold_value": gold_val,
                    "mae": mae,
                    "judge_explain": (f"V3 zero-prediction penalty: pred={pred_val}, gold={gold_val}. "
                                     f"Non-zero gold but zero prediction → min score. {explain}")}
...
async def compute_score_batch_async(data_sources, solution_strs, ground_truths, extra_infos, max_workers=MAX_CONCURRENT_REQUESTS):
    ...
    4. V3: Zero-prediction penalty for regression tasks (non-zero gold, zero pred → min score)

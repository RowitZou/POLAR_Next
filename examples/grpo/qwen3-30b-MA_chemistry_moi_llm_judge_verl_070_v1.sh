...
policy_model_name=Qwen3-30B-A3B-MA
reward_model_name=RULE_LLM_JUDGE_V1
env_name=verl_070_default

# Model paths
actor_path=/mnt/shared-storage-user/ailab-hs/zouyicheng/POLAR/outputs/sft/Qwen3_30B_A3_instruct-general-single/20260107074705/hf-1064
...
reward_manager_path="../src/reward/mix_env/chemistry_reward_manager.py"
...
# Fix glibc TLS assertion failure (ld.so: _dl_allocate_tls_init: Assertion `listp != NULL' failed)
# Caused by too many dynamically loaded shared libraries (CUDA, PyTorch, vLLM, etc.)
...
if [ "$RANK" -eq 0 ]; then 
    ...
    actor_rollout_ref.actor.ppo_max_token_len_per_gpu=$(((2048 + 32768)*1)) \
